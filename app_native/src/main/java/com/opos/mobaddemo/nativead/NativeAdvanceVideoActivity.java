...
public class NativeAdvanceVideoActivity extends Activity implements INativeAdvanceLoadListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initView();
        initData();
        mRootView = (ViewGroup) findViewById(R.id.native_ad_container);
    }
    ...
    public void showAd() {
        ...
        if (null != mINativeAdData && mINativeAdData.isAdValid()) {
            ...
            mINativeAdData.setInteractListener(new INativeAdvanceInteractListener() {
                ...
                @Override
                public void onClick() {
                    Toast.makeText(NativeAdvanceVideoActivity.this.getApplicationContext(), "原生广告点击", Toast.LENGTH_SHORT).show();
                }

                @Override
                public void onShow() {
                    Toast.makeText(NativeAdvanceVideoActivity.this.getApplicationContext(), "原生广告展示", Toast.LENGTH_SHORT).show();
                }

                @Override
                public void onError(int code, String msg) {
                    Toast.makeText(NativeAdvanceVideoActivity.this.getApplicationContext(), "原生广告出错，ret:" + code + ",msg:" + msg, Toast.LENGTH_SHORT).show();
                }
            });
            /**
            ...
            MediaView mediaContainer = (MediaView) container.findViewById(R.id.video_container);
            ...
            /**
             * 绑定广告点击事件与点击按钮，请确保clickViewList.size()大于0
             */
            mINativeAdData.bindToView(this, container, clickViewList);
            //
            if (mINativeAdData.getCreativeType() == INativeAdData.CREATIVE_TYPR_NATIVE_VIDEO) {

                /*
                 * 绑定广告展示的视频View,请在bindMediaView之前调用bindToView
                 */
                mINativeAdData.bindMediaView(this, mediaContainer, new INativeAdvanceMediaListener() {
                    @Override
                    public void onVideoPlayStart() {
                        Log.d(TAG, "onVideoPlayStartReport");
                    }
                ...
                Log.d(TAG, "getVideoDuration  = " + mINativeAdData.getVideoDuration());
            }
        }
    }
    ...
    private void initData() {
         ...
         *构造NativeAd对象。
         */
        mNativeAdvanceAd = new NativeAdvanceAd(this, Constants.NATIVE_ADVANCE_VIDEO_POS_ID, this);
    }
    ...
    public void onAdSuccess(List<INativeAdvanceData> dataList) {
        if (null != dataList && dataList.size() > 0) {
            ...
            Toast.makeText(this.getApplicationContext(), "加载原生广告成功", Toast.LENGTH_SHORT).show();
        }
    }

    @Override
    public void onAdFailed(int ret, String msg) {
        Toast.makeText(this.getApplicationContext(), "加载原生广告失败,错误码：" + ret + ",msg:" + msg, Toast.LENGTH_LONG).show();
    }
